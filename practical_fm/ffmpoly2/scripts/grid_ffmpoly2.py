...
def rough_train( rough ):
    ...
    if rough :
        ...
        return {# 1e-1 1e-2 1e-5 1e-7
                "fl_list" : [1e-1 / pow(10.0,x) for x in range(6)],
                "pl_list" : [1e-1 / pow(10.0,x) for x in range(6)],
                #"k_list" : [4, 8, 16, 32, 64],
                "k_list" : [16],
                "t" : 50,
...
def main():
    ...
    for fl in fl_list:
        for pl in pl_list:
            for k in k_list:
                for v in v_list:
                    for r in r_list:
                        cmd = "{run} -fl {fl} -pl {pl} -k {k} -t {t} -v {v} -r {r} -s {s} -vp {vp} -p {p} {tr}".format(run=solver_path, fl = fl, pl = pl, k = k, t = t ,v = v, r = r, s = s, vp = vp, p = p, tr = tr_path)
    ...
    cmd = "{run} -fl {fl} -pl {pl} -k {k} -t {t} -v {v} -r {r} -s 5 -vp {vp} -p {p} {tr}".format(run=solver_path, fl = min_fl, pl = min_pl, k = min_k, t = t, v = v, r = min_r, vp = final_te_path, p = final_pair_path, tr = final_tr_path)
    print ("Train all.")
    print (cmd)
    proc = subprocess.Popen(cmd, shell=True, stdout = subprocess.PIPE, stderr = subprocess.PIPE)
    out, err = proc.communicate()
