...
void exit_input_error_duplicate(int line_num)
{
...
struct problem vproblem; // Validation Problem
void read_vproblem(const char *filename)
{
  ...
  struct feature_node *vx_space;
	...
	vproblem.y = Malloc(double,vproblem.l);
	vproblem.x = Malloc(struct feature_node *,vproblem.l);
	vx_space = Malloc(struct feature_node,elements+vproblem.l);

	max_index = 0;
	j=0;
	for(i=0;i<vproblem.l;i++)
	{
		inst_max_index = 0; // strtol gives 0 if wrong format
		readline(fp);
		vproblem.x[i] = &vx_space[j];
		label = strtok(line," \t\n");
		if(label == NULL) // empty line
			exit_input_error_duplicate(i+1);

		vproblem.y[i] = strtod(label,&endptr);
		...
		while(1)
		{
			...
			errno = 0;
			vx_space[j].index = (int) strtol(idx,&endptr,10);
			if(endptr == idx || errno != 0 || *endptr != '\0' || vx_space[j].index <= inst_max_index)
				exit_input_error_duplicate(i+1);
			else
				inst_max_index = vx_space[j].index;

			errno = 0;
			vx_space[j].value = strtod(val,&endptr);
		...
		if(vproblem.bias >= 0)
			vx_space[j++].value = vproblem.bias;
	...
	if(vproblem.bias >= 0)
	{
		vproblem.n=max_index+1;
		for(i=1;i<vproblem.l;i++)
			(vproblem.x[i]-2)->index = vproblem.n;
		vx_space[j-2].index = vproblem.n;
	}
	else
		vproblem.n=max_index;
...
void find_parameter_C(const problem *prob, const parameter *param, int nr_fold, double start_C, double max_C, double *best_C, double *best_rate)
{
	...
	//int i;
  char vPath[1000];
  fprintf(stderr,"Enter full path of validation file!\n");
  scanf("%s", vPath);
  read_vproblem(vPath);
  struct problem *vprob = &vproblem;
	double *target = Malloc(double, vprob->l);
  double *loss = Malloc(double, vprob->l);
  ...
  */
	...
	*best_rate = 1e10;
    ...
    param1.init_sol = prev_w;
    struct model *model = train( prob, &param1);

    int total_w_size;
    if(model->nr_class == 2)
      total_w_size = vprob->n;
    else
      total_w_size = vprob->n * model->nr_class;

    if (prev_w == NULL){
      prev_w = Malloc(double, total_w_size);
      ...
      for(int j = 0; j < total_w_size; j++){
        norm_w_diff += (model->w[j] - prev_w[j]) * (model->w[j] - prev_w[j] );
    ...
    for( int j = 0; j < vprob->l; j++){
      double yz = vprob->y[j] * predict(model, vprob->x[j]);
      target[j] = predict_label(model, vprob->x[j]);
      if ( yz > 0 )
        loss[j] = log1p(exp(-1.0 * yz));
      else
        loss[j] =  -yz + log1p(exp(yz));
    }
    ...
    set_print_string_function(default_print_string);
		...
		for(i=0; i<nr_fold; i++)
		{
			...
			for(j=begin; j<end; j++)
				target[perm[j]] = log(1 + exp(-1.0 * prob->y[perm[j]] * predict(submodel,prob->x[perm[j]]) ));
		...
		*/
		...
		double current_logloss = 0.0;
    double current_accuracy = 0.0;
    int total_correct = 0;
		for(int i = 0; i < vprob->l; i++){
			current_logloss += loss[i];
      if( target[i] == vprob->y[i])
        total_correct++;
    }
    current_accuracy = (double) total_correct / (double) vprob->l;
		current_logloss = current_logloss / (double) vprob->l ;
		if(current_logloss < *best_rate)
		{
			*best_C = param1.C;
			*best_rate = current_logloss;
		}

		info("log2c= %7.2f\tlogloss= %g\taccuracy= %g\n",log(param1.C)/log(2.0), current_logloss, current_accuracy);
		num_unchanged_w++;
		if(num_unchanged_w == 10)
			break;
...
double predict_label(const model *model_, const feature_node *x)
{
	double *dec_values = Malloc(double, model_->nr_class);
	double label=predict_values(model_, x, dec_values);
	free(dec_values);
	return (label>0)?model_->label[0]:model_->label[1];
}
