...
#pragma omp parallel for schedule(static) reduction(+: tr_loss)
        ...
        for(ffm_int ii = 0; ii < tr->l; ii++)
        {
            ...
            if( -y*t > 0 ){
              ...
              tr_loss += -y*t + log1p(expyt); // log(1 + e^x) = x + log(1 + e^-x)
              kappa = -y / (1 + expyt);
            }
            else{
              ffm_double expnyt = exp(-y*t);
              tr_loss += log1p(expnyt);
...
#if defined USEOMP
#pragma omp parallel for schedule(static) reduction(+:va_loss, va_accuracy)
#endif
                for(ffm_int i = 0; i < va->l; i++)
                {
                    ...
                    if( -y*t > 0 ){
                      ffm_double expyt = exp(y*t);
                      va_loss += -y*t + log1p(expyt); // log(1 + e^x) = x + log(1 + e^-x)
                    }
                    else{
                      ffm_double expnyt = exp(-y*t);
                      va_loss += log1p(expnyt);
                      va_accuracy += 1;
                    }
                }
                va_loss /= va->l;
